...
* packages/gatekeeper-*: Gatekeeper workers for external service integrations.
    ...
    * A gatekeeper may declare `VendorDescription.autoProvisionsAccount`: it can mint a connected account with no OAuth flow (via `GatekeeperVendor.createAccount()`, which takes no user identity). For such gatekeepers the deployment admin picks a per-vendor mode in the admin Gatekeepers panel â **disabled** / **optional** / **enabled** (default **optional**) â resolved in `provisioning-policy.ts`: `enabled` auto-provisions the account for every user (forced, and hidden from the Connectors list), `optional` lets each user opt in from the Connectors page, and `disabled` offers it to no one (existing accounts go dormant). The Workshop persists the account in the user DO like any connected account (the account capability â not an asserted identity â is the authority thereafter). The **account** (a `GatekeeperUser`) declares in its `AccountDescription` whether it provides an agent **singleton** (`singleton: { tsType }`) and/or a **management UI** (`providesUi`). The Workshop auto-provides the singleton to the owner's workspaces as an **ambient gatekeeper record**, folded into each chat's env as a **named chat binding** (named by the gatekeeper's `suggestedBindingName`; see `prepareChatBindings` in overseer.ts) that the agent reads in `executeCode` (`getSession`/`getAgentCatalog`), each read recorded as an observation. It is not bound to any gadget by default â most gadgets never call it programmatically â but the agent may wire it into a gadget's binding list with `setGadgetBinding` when the gadget's persistent code needs it. The UI is hosted at `/gatekeepers/$appId` (the gatekeeper's vendor id, e.g. `/gatekeepers/context`) via `startAppUi({ isAdmin })`. The two are orthogonal â an account can declare either, both, or neither.
* packages/mcp-shared: Shared implementation behind the two MCP gatekeepers â `gatekeeper-mcp` (endpoints a user pastes) and `gatekeeper-mcp-portal` (one admin-configured portal). Not a Worker; a library both import, holding the MCP client, the OAuth chain, the account DO base, the resource-URL scope grammar, and the queued-action store. See `packages/mcp-shared/README.md` and each connector's README.
    * The trust boundary is `tools.ts`, and nothing outside it reads a tool's annotations: a tool the server declares `readOnlyHint: true` runs as an observation, everything else is queued for approval, and auto-*applying* a write additionally requires a `vetted` endpoint â which only the portal can produce, via `MCP_PORTAL_TRUST_ANNOTATIONS`.
...
Release pipeline (`scripts/release/`) â how customer instances get deployed:

* `build-release.mjs` bundles every deployable worker byte-identically (wrangler dry-run with the pinned wrangler), builds the Access-mode frontend asset build, and generates the release manifest â the contract between this repo's CI and the deploy service, produced by `manifest-lib.mjs` from each package's wrangler.jsonc with account-specific values replaced by placeholders (`$ACCOUNT_ID`, `$WORKER_NAME(...)`, `$SECRET(...)`, `$PUBLIC_BASE_URL`, ...).
...
* Running the flow by hand (upload and promote need `R2_ENDPOINT`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`):
    * `node scripts/release/build-release.mjs --out release-out` â build everything into `release-out/` (id defaults to `r<CI_PIPELINE_IID>-<sha7>` in CI, `dev-<timestamp>` locally; override with `--release-id <id>`).
    * `node scripts/release/upload-release.mjs --release release-out --candidate` â mirror to R2; omit `--candidate` to publish directly (bypasses the gate â CI never does this).
    * `node scripts/release/promote-release.mjs --release-id <id>` â copy the verified candidate's manifest into `releases/<id>/`.
...
- Individual scripts:
    * `pnpm lint:check` / `pnpm lint:fix` â oxlint (config in `.oxlintrc.json`; `correctness` + `suspicious` as errors).
    * `pnpm types:check` â recursive `tsc --noEmit`.
...
- Type-aware oxlint rules are intentionally not enabled. The type-aware engine (tsgo) requires an explicit `rootDir` under declaration emit and drops `baseUrl`, which is incompatible with this monorepo's cross-package source imports. Among other things this means `no-floating-promises` is not enforced â which is just as well, since RPC promise pipelining (below) intentionally leaves promises unawaited. Type safety is still enforced by `tsc` through `pnpm types:check` and `pnpm build`.
...
`proposeFileChange()` or `proposeFileDeletion()` â those bundle everything into the approval
...
**Step 1 â Create a new feature branch (always, before any change).**
...
**Step 2 â Make the change on that branch.**
...
**Step 3 â Verify the change was actually pushed to the branch. Do not skip this.**
...
- `writeFile`/`deleteFile` return a `GitHubCommitHandle`. Call
  ...
  `null`) â `null` means the action is still awaiting approval or hasn't been applied yet.
...
- Independently re-read the branch tip to confirm it advanced:
  ...
  NOT happen â do not proceed to a PR; report the discrepancy and retry.

**Step 4 â Create the pull request, only after verification passes.**
...
**Summary of the rule:** new branch â change on that branch â verify the commit landed on the
branch (handle resolved AND branch SHA advanced) â only then open the PR. Never open a PR for an
unverified push.
...
* You still call the same `writeFile()` / `deleteFile()` methods â the categorization is
...
1. The workflow file exists on the **default branch** (`main`) â not just on your feature
