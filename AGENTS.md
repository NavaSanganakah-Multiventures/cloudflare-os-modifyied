...
1. Choose a meaningful feature-branch name that describes the change (for example
   `feature/add-login`, `fix/compile-error`, or `refactor/db-layer`). The name should reflect
   the user requirement, not a hard-coded issue-based template.
2. Call `proposeFileChange()` for new or updated files, or `proposeFileDeletion()` for removed
   files. These methods create the branch, apply the change, and open a pull request back to the
   default branch.
3. Each step (branch creation, file mutation, and pull-request creation) is submitted to the
