...
export interface JulesListSourcesOptions {
   ...
   *  nextPageToken and returns every matching source. */
  pageSize?: number;
...
export interface JulesListActivitiesOptions {
  /** Number of results per upstream page (max 100). The gatekeeper automatically follows
   *  nextPageToken and returns every matching activity. */
  pageSize?: number;
}
...
export interface JulesCreateSessionInput {
  ...
  title?: string;
  /** The source and context to work against. Required by the Jules API. */
  sourceContext: JulesSourceContext;
...
export interface JulesSession extends RpcTarget {
  ...
  /** List Jules sessions. The gatekeeper follows pagination and returns every session.
   ...
   * @example await session.createSession({ prompt: "Fix the failing tests", sourceContext: { source: "sources/source-id" } }); */
