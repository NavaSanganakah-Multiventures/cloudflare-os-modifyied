...
// Parse the ADMINS config into a list of usernames. ADMINS is ideally a JSON array, but a
// plain comma-separated string (or a single username/email) is also accepted so that a non-JSON
// value like "alice@example.com" doesn't crash admin checks with a JSON parse error.
function parseAdmins(admins: unknown): string[] {
  if (Array.isArray(admins)) {
    return admins.map(a => String(a).trim()).filter(Boolean);
  }
  if (typeof admins === "string") {
    let trimmed = admins.trim();
    if (!trimmed) return [];
    // Prefer the JSON array form, but fall back to a plain comma-separated list.
  ...
  throw new TypeError("ADMINS must be configured as an array of usernames or a comma-separated string.");
}
...
class AuthenticatedApiImpl extends RpcTarget implements AuthenticatedApi {
  ...
  registerPushSubscription(token: string): Promise<void> {
    return this.user.registerPushSubscription(token);
  }

  unregisterPushSubscription(token: string): Promise<void> {
    return this.user.unregisterPushSubscription(token);
  }
