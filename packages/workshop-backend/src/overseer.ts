...
// shape to call it ÃÂ¢ÃÂÃÂ same optional-method-on-a-stub pattern as user.ts's SingletonAccountStub.
...
// observer record. See observers-implementation-plan.md ÃÂÃÂ§3.
type ObserverRecord = {
...
class OverseerImpl implements AgentHooks {
  ...
  // provisional one ÃÂ¢ÃÂÃÂ hooks recorded against it are torn down by removeGadget() if the provisional
  ...
  getEnvForAgent(chatId: number, bindings: Record<string, ChatBindingEntry>): object {
    ...
    for (let [name, entry] of Object.entries(bindings)) {
      ...
      switch (entry.type) {
        ...
        case "value": {
          // Agent callback arguments ÃÂ¢ÃÂÃÂ embed the actual storable args value directly in env.
  ...
  async authorizeObservation(gatekeeperId: number, description: ObservationDescription,
    ...
    // observers-implementation-plan.md ÃÂÃÂ§5 Step 5.
  ...
  // code (which expects a gatekeeper to dereference for approve/reject) never touches it ÃÂ¢ÃÂÃÂ
  ...
  async #runAgentTurnWithContext(chatId: number, aiModel: UserAiModelRecord,
    ...
    try {
      ...
      // emits a stream "clear" ÃÂ¢ÃÂÃÂ otherwise the UI would spin forever on a block.)
      ...
      while (true) {
        ...
        if (hasBeenNudged && liveChat.activeAgentCallbacks.size >= callbackCountBefore) {
          // No progress after being nudged ÃÂ¢ÃÂÃÂ reject remaining callbacks and bail out.
      ...
      // on both the success and error paths ÃÂ¢ÃÂÃÂ an "insufficient funds" failure is exactly when an
  ...
  resolveAgentCallback(chatId: number, sequence: number, value: unknown): void {
    ...
    if (cb) {
      cb.resolve(value);
      // Remove the entry ÃÂ¢ÃÂÃÂ the transient stubs will be invalidated when the
  ...
  // executeCode ÃÂ¢ÃÂÃÂ search/list/read recorded as observations ÃÂ¢ÃÂÃÂ and optionally wire into a gadget
  ...
  // identity to the vendor ÃÂ¢ÃÂÃÂ so the capability is the account the user actually holds.
  async ensureAmbientCapsules(): Promise<void> {
    ...
    // point the capsule at a deleted account ÃÂ¢ÃÂÃÂ so remove it. Snapshot the list since we mutate it.
    ...
    // it needs ÃÂ¢ÃÂÃÂ e.g. account id and sharing domain). We install it as a Facet exactly like any other
    ...
    // that DO ÃÂ¢ÃÂÃÂ only the class reference crosses out.
    //
  ...
  async prepareChatBindings(chatId: number, chatMessages: AiChatMessage[])
    ...
    let {snapshots, changed} = await completeAgentCatalogSnapshot(
        ...
        async gatekeeperId => {
          let record = this.storage.gatekeepers.get(gatekeeperId);
          if (!record) return null;  // disconnected since the chat froze its set ÃÂ¢ÃÂÃÂ no catalog.
          try {
  ...
  // system prompt) call it on every turn ÃÂ¢ÃÂÃÂ caching avoids hammering the user DO each time.
  #vendorsCache: {
    expires: number;
  ...
  async listConnectableResources(vendorId: string): Promise<string> {
    ...
    for (let r of vendor.supportedResources) {
      lines.push(`* ${r.title} ÃÂ¢ÃÂÃÂ urlPattern: ${r.urlPattern}\n  ${r.description}`);
    }
    lines.push(
  ...
  // reason in `message` and the agent should fix it and retry ÃÂ¢ÃÂÃÂ the turn must NOT end (see the
  ...
  async describeStandardFormats(): Promise<string> {
    ...
    return `# Standard output formats\n\n` +
        ...
        formats.map(format =>
            `* ${format.output.noun} (plural: ${format.output.plural}) ÃÂ¢ÃÂÃÂ blueprintId: ` +
  ...
  // graph). See observers-implementation-plan.md ÃÂÃÂ§5 Step 6.
  ...
  //
  // See observers-implementation-plan.md ÃÂÃÂ§5 Step 3.
  ...
  async #describeObserverFailures(
    ...
    let lines = await Promise.all(failed.map(async ({gk, failure}) => {
      ...
      return `${observerBindingTitle(gk)} (${label}) ÃÂ¢ÃÂÃÂ ${oneLineReason(failure.reason)}`;
    }));
...
export class GadgetTailLoopback extends WorkerEntrypoint<Cloudflare.Env, GadgetTailLoopbackProps> {
  ...
  // Do not console.log the tail events here ÃÂ¢ÃÂÃÂ they spam wrangler dev and are not ops logs.
...
class OverseerClientInterface extends RpcTarget implements Overseer {
  ...
  async #maybeResumeAfterActionDecision(chatId: number): Promise<void> {
    ...
    // open across these awaits), but that's cosmetic ÃÂ¢ÃÂÃÂ #resumeSuspendedAgent still starts one turn.
  ...
  async acceptConnectionRequest(
    ...
    this.impl.storage.chats.put(msg);  // fires the subscriber update() ÃÂ¢ÃÂÃÂ re-delivers the card
