...
// shape to call it CatalogGatekeeperFacet.
...
// observer record. See observers-implementation-plan.md for the design.
type ObserverRecord = {
...
class OverseerImpl implements AgentHooks {
  ...
  // provisional one that is discarded if the
  ...
  getEnvForAgent(chatId: number, bindings: Record<string, ChatBindingEntry>): object {
    ...
    for (let [name, entry] of Object.entries(bindings)) {
      ...
      switch (entry.type) {
        ...
        case "value": {
          // Agent callback arguments are storable.
  ...
  async authorizeObservation(gatekeeperId: number, description: ObservationDescription,
    ...
    // observers-implementation-plan.md for details.
  ...
  // code (which expects a gatekeeper to dereference for approve/reject) never touches it ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ built-in
...
5. Use \`writeFile\` on the PR's head branch to commit the resolved code.`;
    ...
    try {
      ...
      while (true) {
        ...
        if (hasBeenNudged && liveChat.activeAgentCallbacks.size >= callbackCountBefore) {
          // No progress after being nudged (reject the callbacks).
  ...
  // identity to the vendor directly.
  async ensureAmbientCapsules(): Promise<void> {
    ...
    // point the capsule at a deleted account (or a replaced one).
  ...
  async prepareChatBindings(chatId: number, chatMessages: AiChatMessage[])
    ...
    let {snapshots, changed} = await completeAgentCatalogSnapshot(
        ...
        async gatekeeperId => {
          let record = this.storage.gatekeepers.get(gatekeeperId);
          if (!record) return null;  // disconnected since the chat froze its set of gatekeepers.
          try {
  ...
  // system prompt) call it on every turn so it stays fresh.
  #vendorsCache: {
    expires: number;
  ...
  async listConnectableResources(vendorId: string): Promise<string> {
    ...
    for (let r of vendor.supportedResources) {
      lines.push(`* ${r.title} ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${r.urlPattern}`)
    }
    lines.push(
  ...
  // reason in `message` and the agent should fix it and retry (see the
  ...
  async describeStandardFormats(): Promise<string> {
    ...
    return `# Standard output formats\n\n` +
        ...
        formats.map(format =>
            `* ${format.output.noun} (plural: ${format.output.plural}) ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ` +
  ...
  // graph). See observers-implementation-plan.md for the design.
  ...
  //
  // See observers-implementation-plan.md for the design.
  ...
  async #describeObserverFailures(
    ...
    let lines = await Promise.all(failed.map(async ({gk, failure}) => {
      ...
      return `${observerBindingTitle(gk)} (${label}) ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${failure.reason}`;
    }));
...
export class GadgetTailLoopback extends WorkerEntrypoint<Cloudflare.Env, GadgetTailLoopbackProps> {
  ...
  // Do not console.log the tail events here (they are user data).
...
class OverseerClientInterface extends RpcTarget implements Overseer {
  ...
  async #maybeResumeAfterActionDecision(chatId: number): Promise<void> {
    ...
    // open across these awaits), but that's cosmetic (duplicate notes are deduped on replay).
